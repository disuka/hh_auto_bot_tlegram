...
# Конфигурация путей
# ----------------------------------------------------------------------
CONFIG_PATH = Path(r"D:\me\hh_auto\api_keys.cfg")
DOCUMENTS_DIR = Path(r"D:\me\hh_auto\my_docs")
...
def load_context_part1():
    if not DOCUMENTS_DIR.exists():
        print("директория контекста (резюме, сопроводительное) не найдена, останов")
        sys.exit(1)

    txt_files = list(DOCUMENTS_DIR.glob("*.txt"))
    if not txt_files:
        print("В папке my_docs нет ни одного .txt файла с документами, останов")
        sys.exit(1)
...
7. иногда ты используешь конструкции, которые ясно дают понять, что ты притворяешься человеком: "Отлично, я готов к собеседованию. Я буду отвечать от первого лица, используя информацию из предоставленных документов, буду краток и профессионален.". не используй такие конструкции. если не понимаешь о чем тебя спрашивают, то просто ответь: "я не понимаю вопроса"
8. не используй в своих ответах уточнения, вида "как указано у меня в резюме" или "В моём резюме указано"
"""
